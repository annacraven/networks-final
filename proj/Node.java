...
public class Node {
    ...
    private int uniquePacketSeqNum;
    ...
    private void receivePacket(int from, Packet packet) {
	    switch(packet.getProtocol()) {
            ...
            case Protocol.TRANSPORT_PKT:
                ...
                this.tcpMan.manOnReceive(from, packet);
                break;
    ...
    public void sendSegment(int srcAddr, int destAddr, int protocol, byte[] payload) {
        //System.out.println("Creating packet...: dstAddr: " + destAddr + " srcAddr: " + srcAddr + " protocol: " + protocol + " payload: " +  payload);

        Packet packet = new Packet(destAddr, srcAddr, Packet.MAX_TTL,
                                   protocol, this.uniquePacketSeqNum, payload);
        this.uniquePacketSeqNum++;
        this.send(destAddr, packet);
    }
    ...
    private boolean matchTransferCommand(String command) {
        ...
        try {
            ...
            SecureSocket sock = new SecureSocket(this.tcpMan);
    ...
    private boolean matchServerCommand(String command) {
        ...
        try {
            ...
            // TODO change this to take a SecureSock, not a tcpSock
            TransferServer server = new
               TransferServer(manager, this, sock.tcpSock, servint, workint, sz);
            server.start();
